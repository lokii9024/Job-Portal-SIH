...
import { FaHome, FaInbox, FaUsers, FaChartBar, FaStar } from "react-icons/fa";
...
const mentorMenu = [
  ...
  },
  {
    label: "Mentoring",
    items: [
      { path: "requests", label: "Student Requests", icon: <FaInbox /> },
      { path: "mentees", label: "Mentees List", icon: <FaUsers /> },
      { path: "placement", label: "Placement Report", icon: <FaChartBar /> },
      { path: "reviews", label: "Reviews About You", icon: <FaStar /> },
    ],
  },
];
    ...
    >
      ...
      <div className="flex items-center justify-between px-3 py-4">
        <div className="flex items-center gap-2">
          ...
          {!collapsed && <span className="font-bold text-indigo-700 text-lg">Mentor</span>}
        </div>
        <button
      ...
      <div className="absolute bottom-0 left-0 right-0 border-t border-gray-200 p-2">
        <button
          ...
          aria-label="Mentor user menu"
        >
          <img
  src="https://images.unsplash.com/photo-1649705433263-5c80d699b5f5?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8N3x8cHJvZmVzb3IlMjBwaG90b3xlbnwwfHwwfHx8MA%3D%3D"
    ...
    <header className="sticky top-0 z-30 border-b border-gray-200 bg-white/80 backdrop-blur">
      <div className="flex items-center gap-3 px-4 py-3">
        ...
        <div className="relative flex-1 max-w-xl">
          ...
          <input
            type="search"
            placeholder="Search requests, mentees, sessions..."
...
function MentorLayout() {
  ...
  useMemo(() => {
    localStorage.setItem("mentor_sidebar_collapsed", String(collapsed));
  }, [collapsed]);
